var Rechnung, data, log, logBuild, logPHP, renderTime, resetSmall, settings, small, theme, time2money, util;
...
time2money = function(minuten, preis) {
  return Math.round(100 * minuten / 60) * preis / 100;
};

renderTime = function(minuten) {};

data = {
  adresse: ["DIS Interim Management GmbH", "c/o DIS AG IT", "Markgrafenstr. 33", "10117 Berlin"],
  ...
  ].map(function(einheit) {
    var h, m, text, _ref;
    _ref = einheit.menge.split(":").map(function(i) {
...
data.summe = data.einheiten.reduce((function(a, b) {
  return a + b.summe;
}), 0);

data.minuten = data.einheiten.reduce((function(a, b) {
  return a + b.minuten;
}), 0);
...
Rechnung(data, function(err, data) {
